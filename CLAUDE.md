...
1. **Fare integrity — CLOSED for booking (2026-08-14), still open for updates.** Booking goes through `book_ride()`, a SECURITY DEFINER RPC that reprices from `fare_config` / `tier_rates` and **refuses the booking if the client's figure disagrees**, so drift surfaces as a visible error instead of an unexpected charge. There is deliberately NO insert policy on `jobs`; restoring one reopens client-priced booking, and a test asserts it stays absent. `config-parity.test.ts` pins the seeded DB rates against `constants.ts` so the two cannot drift unnoticed. **Still open:** `participants update jobs` allows column-level tampering with `quoted_fare` on an existing job (the tip and add-stop paths still write it client-side). Those need the same treatment — an `amend_fare()` RPC — before real money moves.
...
- **Destinations: curated places, not geocoding (decided 2026-08-14 — I chose this; reversible).** Riders book between points in the `places` table rather than typing an address. No external API, no per-lookup cost, works offline, and it suits fixed-fare pricing between known points. Distance is ESTIMATED — straight line × `ROUTE_ESTIMATE.roadFactor`, duration from `averageSpeedKmh` — so **`places` is the service map: if it is empty, nobody can book anything**, and a wrong coordinate is a wrong fare. Where geography intervenes (a bay, a river, a one-way system) the estimate under-reads; fix a persistently bad pair with a measured override for that pair, never by moving the global factor. If geocoding is added later, only `estimateRoute` and the picker change.
- **Tune `roadFactor` (1.35) and `averageSpeedKmh` (26) against real trips before launch** — they set every fare, and the fare lock binds Safeco to them.
...
- **Nobody can perform someone else's action (2026-08-14).** The rider had a "Driver has arrived" button and an "Arrive" button — they could declare the driver at the kerb and jump to an identification screen for a car that had not turned up, or end a trip mid-journey and land on rating and tipping, marking nothing complete. Trip progress is now driven ONLY by the party who owns each step, over realtime status. **When adding a screen, ask who can actually know the thing the control asserts.**
...
## Known workflow hazard (release builds in the monorepo)

Gradle's JS-bundling task (`createBundleReleaseJsAndAssets`) does NOT track `packages/shared` as an input. Change shared code, rebuild the app, and gradle reports the bundle "up-to-date" — **the APK ships the stale bundle with no warning** (shipped a fixed-crash build that still crashed, 2026-08-14). Before installing a release APK after shared-package changes: delete `android/app/build/generated/assets/react` + `android/app/build/intermediates/assets/release`, rebuild, and verify the fix is actually inside — string literals stay readable in Hermes bytecode, so `node -e "console.log(require('fs').readFileSync('app-release.apk').includes(Buffer.from('SOME_NEW_LITERAL')))"` works as a check.
