...
- **Still placeholders inside live screens** (each marked in code): driver earnings/day-ledger figures (need a query over completed jobs), candidate ETAs (3/6/9 by index — need telemetry), `avgAssignSeconds` and `returnedToday` (need the `job_events` timeline), and the recents list on Plan.
